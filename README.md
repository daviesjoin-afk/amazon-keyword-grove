...
**Version 0.3.2 · MIT License · Local-first**

Keyword Grove 是一个本地优先的亚马逊 SellerSprite 关键词反查表管理工具。它面向多个产品和竞品 ASIN，使用产品标题、五点、核心词根、内置规则和 AI 语义审核生成可下载的广告建议草稿。
...
- AI 接口在工作台的“AI 语义设置”中配置：默认优先使用 OpenRouter 免费 MiniMax M3，失败时回退到免费 MiniMax M2.7；回退模型可关闭、自定义或切换预设，API Key 只写入本地数据库且不会回显
...
Windows 用户也可以直接双击项目根目录的 [`启动关键词库.cmd`](./启动关键词库.cmd)。它会检查并启动本地服务，然后自动打开管理页面；不会触发任何 AI 审核任务。
...
AI 服务的 API Key 请在页面的“AI 语义设置”中填写，不要写入源码、README、Issue 或提交记录。读取配置时只返回脱敏信息。
...
- 侧边栏提供最多三个产品的快捷切换入口；切换结果保存在本地浏览器，刷新后会继续打开最近选中的产品。
- OpenRouter/OpenAI-compatible 语义审核属于辅助层，关键广告判断仍保留规则保护与人工审批。
