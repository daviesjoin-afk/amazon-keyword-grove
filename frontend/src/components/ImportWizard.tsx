...
function PreviewStep({ product }: { product: Product }) {
  ...
  return <div className="wizard-step-content"><div className="wizard-intro compact-intro"><span className="step-ordinal">03 / PREVIEW</span><h2>预览清洗后的数据</h2><p>以产品标题和五点为语料进行首轮语义评分，同时用竞品 ASIN 占比展示相关性。关键词只会归属到当前产品，不会跨产品污染。</p></div><div className="preview-info-strip"><span><strong>{product.name}</strong><small>{product.site} · 竞品参考 {product.referenceAsin}</small></span><span><b>2,000</b> 行待处理</span><span><b>20</b> 个来源 ASIN</span><span className="preview-warning"><Info size={14} />缺失字段保留为未知</span></div><div className="preview-table-wrap"><table className="preview-table"><thead><tr><th>关键词</th><th>翻译</th><th>月搜索量</th><th>相关性（竞品占比）</th><th>流量类型</th><th>语义评分</th></tr></thead><tbody>{rows.map((row) => <tr key={row[0]}><td><strong>{row[0]}</strong></td><td>{row[1]}</td><td>{row[2]}</td><td>{row[3]}</td><td><span className="traffic-types">{row[4].split(' / ').map((item) => <span key={item}>{item}</span>)}</span></td><td><strong className={Number(row[5]) >= 80 ? 'score-good' : Number(row[5]) >= 50 ? 'score-mid' : 'score-bad'}>{row[5]}</strong></td></tr>)}</tbody></table></div><div className="preview-footnote"><Info size={14} /><span>预览仅显示前 4 行。相关性按当前竞品 ASIN 占比计算（例如 5/20）；语义评分仅作为双重审核证据。</span></div></div>
}
...
function AnalysisStep({ analyzeMode, setAnalyzeMode, dedupe, setDedupe }: { analyzeMode: 'rule' | 'assist'; setAnalyzeMode: (mode: 'rule' | 'assist') => void; dedupe: boolean; setDedupe: (value: boolean) => void }) {
  return <div className="wizard-step-content"><div className="wizard-intro compact-intro"><span className="step-ordinal">04 / ANALYSIS</span><h2>设置清洗与分析方式</h2><p>导入时先执行内置规则预审；完成导入后，必须在产品工作台运行 AI 全量审核，最终投放和否词草稿才算双重审核结果。</p></div><div className="analysis-option-grid"><label className={`analysis-option ${analyzeMode === 'rule' ? 'is-selected' : ''}`}><input type="radio" name="analyze-mode" checked={analyzeMode === 'rule'} onChange={() => setAnalyzeMode('rule')} /><span className="analysis-option-icon"><CheckCircle2 size={18} /></span><span><strong>内置规则预审</strong><small>使用标题、五点、核心词根和冲突词完成首轮相关性分析；结果会等待 AI 二审。</small></span><em>必经</em></label><label className={`analysis-option ${analyzeMode === 'assist' ? 'is-selected' : ''}`}><input type="radio" name="analyze-mode" checked={analyzeMode === 'assist'} onChange={() => setAnalyzeMode('assist')} /><span className="analysis-option-icon"><RefreshCw size={18} /></span><span><strong>AI 全量二审</strong><small>在工作台按每批 40 条发送全部关键词，结合产品资料和内置规则证据生成最终草稿。</small></span><em className="option-beta">必需</em></label></div><div className="analysis-checklist"><label><input type="checkbox" checked={dedupe} onChange={(event) => setDedupe(event.target.checked)} /><span><strong>按产品 + 站点 + 标准化关键词去重</strong><small>英文统一小写、清理首尾空格与不可见字符，不执行词干化。</small></span></label><label><input type="checkbox" defaultChecked /><span><strong>保留所有原始扩展字段</strong><small>未知列和无法解析值仍可在详情抽屉查看。</small></span></label><label><input type="checkbox" defaultChecked /><span><strong>生成双审广告建议草稿</strong><small>最终只保留精准、广泛、观察、否定精准和否定词组；结果仍需人工审批。</small></span></label></div></div>
}

function ReportStep({ fileName }: { fileName: string }) {
  return <div className="wizard-step-content report-step-content"><div className="report-success-icon"><CheckCircle2 size={28} /></div><span className="step-ordinal">05 / REPORT</span><h2>导入准备完成</h2><p><strong>{fileName}</strong> 已通过结构校验，下面是将写入产品词库的处理摘要。</p><div className="report-grid"><div><b>2,000</b><span>总行数</span></div><div><b className="report-green">2,000</b><span>新增关键词</span></div><div><b>0</b><span>重复跳过</span></div><div><b className="report-amber">待 AI 二审</b><span>最终决策</span></div></div><div className="report-next-step"><div className="callout-icon"><Check size={17} /></div><div><strong>下一步</strong><p>系统会将本批次写入当前产品并保留导入记录。返回工作台后点击“AI 全量审核”，全部关键词通过内置规则和 AI 双重审核后，才生成最终投放/否词草稿。</p></div></div><div className="report-note"><Info size={14} />上传的 ASIN 会标记为竞品参考来源，不会被当作自有产品。</div></div>
}
