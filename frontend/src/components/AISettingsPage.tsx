...
export function AISettingsPage() {
  ...
  return <div className="product-settings-page ai-workspace-page">
    ...
    <section className="settings-card ai-settings-card">
      <div className="settings-card-head"><div className="settings-card-icon ai-icon"><Bot size={20} /></div><div><h2>AI 语义模型接口</h2><p>默认使用已配置的 OpenAI 兼容模型接口，用于语义分类增强，不会自动执行广告或否定操作。</p></div><label className="settings-switch"><input type="checkbox" checked={ai.enabled} onChange={(event) => setAI((current) => ({ ...current, enabled: event.target.checked }))} /><span />启用配置</label></div>
      {loading ? <div className="settings-loading">正在读取本地配置…</div> : <div className="ai-form-grid">
        <label className="settings-field"><span>接口类型</span><select value={ai.provider} onChange={(event) => selectProvider(event.target.value)}><option value="mimo">默认兼容接口</option><option value="openai_compatible">OpenAI 兼容接口</option><option value="openai">OpenAI</option><option value="deepseek">DeepSeek</option><option value="custom">自定义</option></select></label>
        <label className="settings-field"><span>模型名称 <em>*</em></span><input value={ai.model} onChange={(event) => setAI((current) => ({ ...current, model: event.target.value }))} placeholder="例如 provider/model-v2" /></label>
