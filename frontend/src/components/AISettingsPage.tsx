...
export function AISettingsPage() {
  ...
  return <div className="product-settings-page ai-workspace-page">
    ...
    <section className="settings-card ai-settings-card">
      <div className="settings-card-head"><div className="settings-card-icon ai-icon"><Bot size={20} /></div><div><h2>{text('AI 语义模型接口', 'AI semantic model endpoint')}</h2><p>{text('默认使用小米 MiMo V2.5 的 OpenAI 兼容接口，用于后续语义分类增强，不会自动执行广告或否定操作。', 'The default preset uses Xiaomi MiMo V2.5 through an OpenAI-compatible endpoint. Semantic review augments classification and never auto-executes ads or negatives.')}</p></div><label className="settings-switch"><input type="checkbox" checked={ai.enabled} onChange={(event) => setAI((current) => ({ ...current, enabled: event.target.checked }))} /><span />{text('启用配置', 'Enable')}</label></div>
      {loading ? <div className="settings-loading">{text('正在读取本地配置…', 'Loading local configuration…')}</div> : <div className="ai-form-grid">
        <label className="settings-field"><span>{text('接口类型', 'Provider')}</span><select value={ai.provider} onChange={(event) => selectProvider(event.target.value)}><option value="mimo">{text('小米 MiMo（默认）', 'Xiaomi MiMo (default)')}</option><option value="openai_compatible">OpenAI Compatible</option><option value="openai">OpenAI</option><option value="deepseek">DeepSeek</option><option value="custom">{text('自定义', 'Custom')}</option></select></label>
        <label className="settings-field"><span>{text('模型名称', 'Model name')} <em>*</em></span><input value={ai.model} onChange={(event) => setAI((current) => ({ ...current, model: event.target.value }))} placeholder="e.g. mimo-v2.5" /></label>
        <label className="settings-field full-settings-field"><span>API Base URL <em>*</em></span><input value={ai.baseUrl} onChange={(event) => setAI((current) => ({ ...current, baseUrl: event.target.value }))} placeholder="https://api.xiaomimimo.com/v1" /></label>
