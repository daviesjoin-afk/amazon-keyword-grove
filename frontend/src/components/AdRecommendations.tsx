...
export function AdRecommendations({ product, keywords, onSelectKeyword, onSemanticReview, semanticReviewing, reviewProgress }: AdRecommendationsProps) {
  ...
  const pendingBroad = keywords.filter((keyword) => !keyword.semanticReviewed && (keyword.suggestedAction === '广泛探索' || keyword.broadRootCandidate)).length
  const pendingNegativePhrase = keywords.filter((keyword) => !keyword.semanticReviewed && keyword.suggestedAction === '否定词组').length
  const conflictRows = keywords.filter((keyword) => keyword.semanticReviewed && keyword.hasActionConflict)
  const broadOverflow = keywords.filter((keyword) => keyword.broadRootCandidate && (keyword.broadRootRank || 0) > 10).length
  ...
  function download(group: RecommendationGroup, rows: KeywordRecord[]) {
    const headers = [
      ...
      text('置信度', 'Confidence'),
      text('风险', 'Risk'),
      text('最终动作来源', 'Final action source'),
      text('规则版本', 'Rule version'),
      text('词根', 'Root'),
      text('理由', 'Reason'),
      text('月搜索量', 'Monthly search volume'),
    ]
    const values = rows.map((item) => [item.keyword, relevanceRatio(item), item.relevanceScore, item.confidence, item.risk, item.finalActionSource || '', item.ruleEngineVersion || '', item.negativePhraseRoot || '', item.suggestionReason, item.monthlySearchVolume ?? ''])
  ...
  const statusDetail = semanticReviewing && productReview
    ...
    : productReview?.status === 'partial'
      ...
      : productReview?.status === 'failed'
        ...
        : pending
          ? text(`待 AI 二审 ${pending.toLocaleString(numberLocale)} 条`, `${pending.toLocaleString(numberLocale)} awaiting AI review`)
  ...
  return <div className="ad-recommendation-page">
    <header className="ad-recommendation-head">
      <div><span className="panel-kicker">{text('Advertising playbook / 双重审核', 'Advertising playbook / Two-stage review')}</span><h1>{text('广告建议', 'Ad Suggestions')}</h1><p>{text('内置规则先筛选，AI 再结合标题、五点和词根做语义复核。这里仅展示可导出的最终动作草稿。', 'Deterministic rules screen first; AI then reviews title, bullets, roots, and rule evidence. Only exportable final-action drafts appear here.')}</p></div>
      <div className="ad-recommendation-actions"><button className="button button-primary compact-button" type="button" disabled={semanticReviewing || pending === 0} title={text('一次处理全部尚未完成 AI 二审的关键词；服务端按有界批次并发调用，不重复审核已完成结果', 'Review all keywords still pending AI review using bounded concurrent batches without reprocessing completed results')} onClick={() => void onSemanticReview('incremental')}><RefreshCw size={15} />{semanticReviewing ? text('增量审核中…', 'Reviewing…') : pending ? text('增量审核', 'Incremental review') : text('已全部审核', 'All reviewed')}</button><button className="button button-secondary compact-button" type="button" disabled={semanticReviewing || total === 0} title={text('重新审核所有未人工锁定的关键词，会产生新的模型调用；人工锁定结果保持不变', 'Re-review all keywords except manual locks; this creates new model calls while preserving manual decisions')} onClick={() => void onSemanticReview('full')}><RefreshCw size={15} />{text('重新审核', 'Re-review')}</button><div className="ad-review-status"><span className={pending ? 'status-dot status-dot-pending' : 'status-dot status-dot-live'} /><strong>{text(`已审核 ${reviewed.toLocaleString(numberLocale)} / ${total.toLocaleString(numberLocale)}`, `Reviewed ${reviewed.toLocaleString(numberLocale)} / ${total.toLocaleString(numberLocale)}`)}</strong><span>{statusDetail}</span></div></div>
    </header>
    {pending > 0 && <div className="ad-pending-banner"><Eye size={17} /><span>{semanticReviewing && productReview ? <>{text('AI 正在审核：已完成', 'AI review in progress:')} <strong>{productReview.batches_completed}</strong> / <strong>{productReview.batches_total}</strong> {text('批，已审核', 'batches; reviewed')} <strong>{reviewed.toLocaleString(numberLocale)} / {total.toLocaleString(numberLocale)}</strong>. {text('页面刷新后会自动恢复进度。', 'Progress is restored after refresh.')}</> : <>{text('当前还有', 'There are')} <strong>{pending.toLocaleString(numberLocale)}</strong> {text('条只有内置规则预审，暂不进入四类导出清单。规则预审已标出', 'keywords with deterministic pre-review only; they are excluded from export lists. Rule pre-review currently marks')} <strong>{pendingBroad.toLocaleString(numberLocale)}</strong> {text('条广泛候选、', 'broad candidate(s) and')} <strong>{pendingNegativePhrase.toLocaleString(numberLocale)}</strong> {text('条否定词组候选；点击“增量审核”可继续处理。', 'negative-phrase candidate(s). Run incremental review to continue.')}</>}</span></div>}
    <div className="ad-recommendation-grid">
      {groups.map((group) => {
        const rows = keywords.filter((item) => item.suggestedAction === group.key && item.semanticReviewed && !item.hasActionConflict).sort((left, right) => (right.monthlySearchVolume ?? -1) - (left.monthlySearchVolume ?? -1))
        ...
        return <section className={`ad-recommendation-card ad-tone-${group.tone}`} key={group.key}>
          ...
          <div className="ad-card-meta"><strong>{rows.length.toLocaleString(numberLocale)}</strong><span>{text('条建议词', 'suggestions')}</span>{group.key === '广泛探索' && broadOverflow > 0 && <span className="ad-overflow-note">{text(`另有 ${broadOverflow} 条超过上限`, `${broadOverflow} more exceed the cap`)}</span>}<button className="text-button" type="button" onClick={() => download(group, rows)}><FileDown size={13} />{text('下载 CSV', 'Download CSV')}</button></div>
          <div className="ad-suggestion-list">{rows.length ? rows.map((item) => <button className="ad-suggestion-row" type="button" key={item.id} onClick={() => onSelectKeyword(item)}><span className="ad-suggestion-main"><strong>{item.keyword}</strong><small>{item.suggestionReason}{item.negativePhraseRoot ? ` · 词根：${item.negativePhraseRoot}` : ''}</small></span><span className="ad-suggestion-metric"><b>{relevanceRatio(item)}</b><small>{text('相关性', 'relevance')}</small></span><ConfidencePill value={item.confidence} /></button>) : <div className="ad-empty"><CheckCircle2 size={17} /><span>{pending > 0 ? text('完成 AI 二审后，符合条件的建议会显示在这里。', 'Eligible suggestions will appear here after AI review.') : text('本次双重审核没有通过该动作的安全门槛，当前无可导出建议词。', 'No keyword passed the safety threshold for this action in the current two-stage review.')}</span></div>}</div>
        </section>
      })}
    </div>
    {conflictRows.length > 0 && <section className="ad-conflict-card"><div className="ad-card-head"><div className="ad-card-title"><span className="ad-card-icon ad-conflict-icon"><ShieldAlert size={17} /></span><div><span className="panel-kicker">Action conflicts / 人工复核</span><h2>{text('动作冲突', 'Action conflicts')}</h2></div></div><strong>{conflictRows.length.toLocaleString(numberLocale)} {text('条', '')}</strong></div><p className="ad-card-description">{text('同一关键词同时出现人工动作与自动建议，已从四类导出清单排除。请打开详情确认后再锁定最终动作。', 'Keywords with conflicting manual and automatic actions are excluded from exports. Open each detail and lock the final action.')}</p><div className="ad-suggestion-list">{conflictRows.map((item) => <button className="ad-suggestion-row" type="button" key={item.id} onClick={() => onSelectKeyword(item)}><span className="ad-suggestion-main"><strong>{item.keyword}</strong><small>冲突动作：{(item.conflictActions || []).join(' / ')} · 规则版本：{item.ruleEngineVersion || '未记录'}</small></span><span className="ad-suggestion-metric"><b>{relevanceRatio(item)}</b><small>{text('相关性', 'relevance')}</small></span><ConfidencePill value={item.confidence} /></button>)}</div></section>}
