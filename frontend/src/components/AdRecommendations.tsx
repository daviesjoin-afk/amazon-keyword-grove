...
export function AdRecommendations({ product, keywords, onSelectKeyword, onSemanticReview, semanticReviewing, reviewProgress }: AdRecommendationsProps) {
  ...
  function download(group: RecommendationGroup, rows: KeywordRecord[]) {
    const headers = ['关键词', '相关性（竞品占比）', '语义评分', '置信度', '风险', '最终动作来源', '规则版本', '词根', '理由', '月搜索量']
  ...
  return <div className="ad-recommendation-page">
    <header className="ad-recommendation-head">
      ...
      <div className="ad-recommendation-actions"><button className="button button-primary compact-button" type="button" disabled={semanticReviewing || pending === 0} title="一次处理全部尚未完成 AI 二审的关键词；服务端按批次并发调用，不重复审核已完成结果" onClick={() => void onSemanticReview('incremental')}><RefreshCw size={15} />{semanticReviewing ? productReview?.review_mode === 'full' ? '重新审核中…' : '增量审核中…' : pending ? '增量审核' : '已全部审核'}</button><button className="button button-secondary compact-button" type="button" disabled={semanticReviewing || total === 0} title="重新审核所有未人工锁定的关键词，会产生新的模型调用；人工锁定结果保持不变" onClick={() => void onSemanticReview('full')}><RefreshCw size={15} />重新审核</button><div className="ad-review-status"><span className={pending ? 'status-dot status-dot-pending' : 'status-dot status-dot-live'} /><strong>已审核 {reviewed.toLocaleString('en-US')} / {total.toLocaleString('en-US')}</strong><span>{semanticReviewing && productReview ? `正在处理第 ${Math.min(productReview.batches_completed + 1, productReview.batches_total)} / ${productReview.batches_total} 批（已完成 ${productReview.batches_completed} 批）` : productReview?.status === 'partial' ? `审核完成但有 ${productReview.failed_batches.length} 批失败，可继续增量重试` : productReview?.status === 'failed' ? `审核失败：${productReview.error || '请检查 AI 设置'}` : pending ? `待 AI 二审 ${pending.toLocaleString('en-US')} 条` : '双重审核完成，已形成建议'}</span></div></div>
    </header>
    ...
    <div className="ad-recommendation-grid">
      {groups.map((group) => {
        ...
        return <section className={`ad-recommendation-card ad-tone-${group.tone}`} key={group.key}>
          ...
          <div className="ad-card-meta"><strong>{rows.length.toLocaleString('en-US')}</strong><span>条建议词</span>{group.key === '广泛探索' && broadOverflow > 0 && <span className="ad-overflow-note">另有 {broadOverflow} 条超过上限</span>}<button className="text-button" type="button" onClick={() => download(group, rows)}><FileDown size={13} />下载 CSV</button></div>
          <div className="ad-suggestion-list">{rows.length ? rows.map((item) => <button className="ad-suggestion-row" type="button" key={item.id} onClick={() => onSelectKeyword(item)}><span className="ad-suggestion-main"><strong>{item.keyword}</strong><small>{item.suggestionReason}{item.negativePhraseRoot ? ` · 词根：${item.negativePhraseRoot}` : ''}</small></span><span className="ad-suggestion-metric"><b>{relevanceRatio(item)}</b><small>相关性</small></span><ConfidencePill value={item.confidence} /></button>) : <div className="ad-empty"><CheckCircle2 size={17} /><span>{pending > 0 ? '完成 AI 二审后，符合条件的建议会显示在这里。' : '本次双重审核没有通过该动作的安全门槛，当前无可导出建议词。'}</span></div>}</div>
        </section>
      })}
    </div>
    {conflictRows.length > 0 && <section className="ad-conflict-card"><div className="ad-card-head"><div className="ad-card-title"><span className="ad-card-icon ad-conflict-icon"><ShieldAlert size={17} /></span><div><span className="panel-kicker">Action conflicts / 人工复核</span><h2>动作冲突</h2></div></div><strong>{conflictRows.length.toLocaleString('en-US')} 条</strong></div><p className="ad-card-description">同一关键词同时出现人工动作与自动建议，已从四类导出清单排除。请打开详情确认后再锁定最终动作。</p><div className="ad-suggestion-list">{conflictRows.map((item) => <button className="ad-suggestion-row" type="button" key={item.id} onClick={() => onSelectKeyword(item)}><span className="ad-suggestion-main"><strong>{item.keyword}</strong><small>冲突动作：{(item.conflictActions || []).join(' / ')} · 规则版本：{item.ruleEngineVersion || '未记录'}</small></span><span className="ad-suggestion-metric"><b>{relevanceRatio(item)}</b><small>相关性</small></span><ConfidencePill value={item.confidence} /></button>)}</div></section>}
