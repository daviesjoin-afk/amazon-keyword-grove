...
- 产品中心为每个产品卡片增加明确的“删除”按钮和确认归档入口；当前产品在产品中心和侧栏均以绿色边框、背景和状态徽标区分，删除当前产品后会安全切换到其他产品。
- 侧边栏新增最多三个产品的快捷切换列表；点击产品卡片会立即切换当前词库，当前产品始终保留在列表内，刷新页面后通过已保存的产品 ID恢复选择。
- AI 语义设置明确区分主模型与回退模型：默认主模型为 OpenRouter 免费 MiniMax M3，默认回退模型为免费 MiniMax M2.7；支持关闭或自定义回退模型，主模型请求失败时才自动切换。

### Verification

- Backend: `20 passed, 1 skipped`；覆盖重复否定精准词根、有效扩展误伤保护、增量持久化、全量重新审核、人工锁定保护、后台进度与主模型失败后的回退模型。
- Frontend: `16 passed`，并通过 TypeScript 与 Vite production build。
