...
ALLOWED_PRODUCT_STATUSES = {"preparing", "active", "on_sale", "archived"}
ALLOWED_KEYWORD_STATUSES = {"待评估", "Listing 已使用", "广告待投放", "广告已投放", "观察", "否定候选", "已否定", "不相关", "active", "deleted"}
ALLOWED_ACTIONS = {"exact", "broad", "observe", "negative_exact", "negative_phrase", "manual_review", "insufficient_data"}
ACTION_LABELS = {
    "exact": "建议精准投放",
    "broad": "建议广泛测试",
    "observe": "观察/暂不投放",
...
def _product_response(row: Any, connection: Any) -> dict[str, Any]:
    data = dict(row)
...
def _keyword_response(row: Any, connection: Any | None = None) -> dict[str, Any]:
    data = dict(row)
    for key in ("traffic_types_json", "related_asins_json", "top_10_asins_json", "matched_terms_json", "conflicting_terms_json", "classification_reason_json", "advice_data_basis_json", "manual_tags_json"):
    ...
    if connection is not None:
        source_rows = connection.execute("SELECT asin, first_import_id, last_import_id, first_seen_at, last_seen_at FROM keyword_sources WHERE keyword_id = ? ORDER BY asin", (data["id"],)).fetchall()
...
def _get_product(connection: Any, product_id: int, *, include_deleted: bool = False) -> Any:
    row = connection.execute("SELECT * FROM products WHERE id = ?", (product_id,)).fetchone()
    if row is None or (row["deleted_at"] and not include_deleted):
        _api_error(404, "product_not_found", "产品不存在")
    return row


def _parse_mapping(mapping_json: str | None) -> dict[str, Any] | None:
    if not mapping_json:
        return None
    try:
        parsed = json.loads(mapping_json)
    except (TypeError, ValueError):
        _api_error(400, "invalid_mapping", "mapping_json 不是有效 JSON")
    if not isinstance(parsed, dict):
        _api_error(400, "invalid_mapping", "字段映射必须是对象")
    return parsed
...
def _keyword_update_parts(values: dict[str, Any]) -> tuple[list[str], list[Any]]:
...
def test_ai_config() -> dict[str, Any]:
    ...
    if not config.get("enabled"):
        _api_error(422, "ai_not_enabled", "请先启用 AI 配置")
    if not api_key:
        _api_error(422, "missing_ai_key", "请先填写 API Key")
    if not base_url or not model:
        _api_error(422, "missing_ai_config", "请先填写接口地址和模型名称")
    ...
    try:
        with urllib.request.urlopen(request, timeout=min(max(int(config.get("timeout_seconds") or 60), 5), 60)) as response:
            raw = response.read().decode("utf-8")
            result = json.loads(raw)
    except urllib.error.HTTPError as exc:
        detail = exc.read().decode("utf-8", errors="replace")[:500]
        _api_error(502, "ai_test_rejected", f"模型接口拒绝测试请求（HTTP {exc.code}）：{detail}")
    except (urllib.error.URLError, TimeoutError, ValueError) as exc:
        _api_error(502, "ai_test_failed", f"模型接口连接失败：{str(exc)[:300]}")
    ...
    try:
        ...
        _api_error(502, "ai_test_invalid_response", "模型接口返回格式无法识别")
    return {"ok": True, "provider": clean_text(config.get("provider")), "model": model, "reply": content[:200]}


def _ai_config_or_error() -> dict[str, Any]:
    with read_connection() as connection:
        row = connection.execute("SELECT value_json FROM app_settings WHERE key = 'ai_config'").fetchone()
    config = loads(row["value_json"], {}) if row else {}
    if not config.get("enabled"):
...
def _ai_json_response(config: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    base_url = clean_text(config.get("base_url")).rstrip("/")
    api_key = clean_text(config.get("api_key"))
    request = urllib.request.Request(
        f"{base_url}/chat/completions",
    ...
    try:
        ...
        _api_error(502, "ai_review_rejected", f"模型接口拒绝审核请求（HTTP {exc.code}）")
    except (urllib.error.URLError, TimeoutError, ValueError) as exc:
        _api_error(502, "ai_review_failed", f"模型接口连接失败：{str(exc)[:300]}")
    try:
        content = clean_text(result["choices"][0]["message"]["content"])
        start, end = content.find("{"), content.rfind("}")
        return json.loads(content[start:end + 1])
    except (KeyError, IndexError, TypeError, ValueError, json.JSONDecodeError):
        _api_error(502, "ai_review_invalid_response", "模型未返回可解析的 JSON 审核结果")


def _semantic_review_signature(product: dict[str, Any], candidate: dict[str, Any]) -> str:
    """Return a stable fingerprint for a rule+MiMo decision."""
...
def _semantic_reason_is_too_broad(reason: str) -> bool:
    """Detect an explicit model warning that a query is too generic.

    MiMo returns free-form Chinese reasons, so the decision must not rely on a
    single structured field.  These markers only act as a downgrade guard for
    non-core queries; they never promote or create an advertising action.
    """

    text = clean_text(reason).casefold()
    markers = (
        "太宽泛", "过宽泛", "较宽泛", "过宽", "太泛", "泛词", "不够具体",
        "缺乏具体", "缺少具体", "转化率可能不如更具体", "too broad", "overly broad",
        "too generic", "overly generic", "not specific enough", "less specific",
    )
    return any(marker in text for marker in markers)


def _is_short_generic_query(keyword: str, product: dict[str, Any]) -> bool:
    """Return whether a short query lacks a product anchor.

    This mirrors the narrow local-rule gate for semantic results.  It is kept
    here as a final safety check because MiMo may raise a weak local score for
    a generic query while still returning an exact decision.
    """
